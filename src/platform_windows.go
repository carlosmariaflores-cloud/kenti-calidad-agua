...
func openWindow(url string) <-chan struct{} {
	...
	for _, b := range browsers() {
		...
		cmd := exec.Command(b, "--app="+url, "--user-data-dir="+profile, "--no-first-run", "--no-default-browser-check", "--disable-sync", "--window-size=1366,900")
...
func openFolder(dir string) { exec.Command("explorer", dir).Start() }
