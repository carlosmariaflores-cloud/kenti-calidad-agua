#!/usr/bin/env python3
"""El ícono de Windows como recurso COFF (.syso), sin dependencias externas (igual que en los otros módulos)."""
import io
import struct
...
<assembly xmlns="urn:schemas-microsoft-com:asm.v1" manifestVersion="1.0">
  <assemblyIdentity type="win32" name="Kenti.App" version="0.1.0.0"/>
...
def build_rsrc(leaves):
    ...
    lang_off = []
    for _ in leaves:
        lang_off.append(off); off += 16 + 8
    de_off = []
    for _ in leaves:
        de_off.append(off); off += 16
    data_off = []
    for _, _, d in leaves:
        off = (off + 7) & ~7
...
def escribir_syso(png, syso):
    ...
    coff += rsrc
    for r in relocs:
        coff += struct.pack("<IIH", r, 0, 0x0003)
...
if __name__ == "__main__":
    escribir_syso("web/icono.png", "rsrc_windows_amd64.syso")
